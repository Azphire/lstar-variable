...
class EquivalenceQuery:
    ...
    def find_counter_example(self, prod_m: Machine, m1: Machine, m2: Machine,
                             state: Tuple, pre: str, v1: int, v2: int, visited: list) -> bool:
    ...
    def query(self) -> Tuple[bool, str]:
        ...
        if self.find_counter_example(prod_1, self.m, self.reversed_h, prod_1.start, '', 0, 0, []):
            return False, self.counter_example
        prod_2 = self.sync_product(self.h, self.reversed_m)
        if self.find_counter_example(prod_2, self.h, self.reversed_m, prod_2.start, '', 0, 0, []):
            return False, self.counter_example
        return True, ''
