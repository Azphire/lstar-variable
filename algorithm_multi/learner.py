    ...
    def build_dfa(self):
        ...
        for i in range(state_num):
            ...
            for char in self.teacher.alphabet:
                ...
                dfa_map[i][char] = []
        self.learning_machine = Machine(self.teacher.alphabet, dfa_states, dfa_map, 0,
                                        self.obTable.T[0][0][1], accepted)
        # 遍历每一列S，更新dfa
        for i in range(len(self.obTable.S)):
            ...
            for char in self.obTable.alphabet:
                ...
                guards = self.obTable.T[self.obTable.S.index(s)][0][1]
                assignments = self.obTable.T[self.obTable.S.index(sentence)][0][1]
                self.learning_machine.update_once(current_state, char,
                                                  Trans(guards, target_state, assignments))
